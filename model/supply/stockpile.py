...
class StockpileSupply:
    ...
    def update_stockpile(self, year: int, net_balance: float) -> None:
        """
        Update stockpile for a specific year based on net balance.
        
        Args:
            year: Year to update.
            net_balance: Net balance (supply minus demand) for the year.
                Negative values indicate drawing from stockpile.
                Positive values indicate adding to stockpile.
        """
        ...
        # initialise units used
        units_used_from_stockpile = 0
        surplus_used = 0
        non_surplus_used = 0
        forestry_held_addition = 0
        forestry_surrender_addition = 0
        
        # Add forestry_held and forestry_surrender to non-surplus stockpile if available
        if self.forestry_variables is not None and year in self.forestry_variables.index:
            # Get forestry_held value for this year
            if 'forestry_held' in self.forestry_variables.columns:
                forestry_held = self.forestry_variables.loc[year, 'forestry_held']
                if not pd.isna(forestry_held) and forestry_held > 0:
                    forestry_held_addition = forestry_held
                    self.stockpile_balance[year] += forestry_held_addition
            
            # Get forestry_surrender value for this year (typically negative)
            if 'forestry_surrender' in self.forestry_variables.columns:
                forestry_surrender = self.forestry_variables.loc[year, 'forestry_surrender']
                if not pd.isna(forestry_surrender):
                    # Surrender values are typically negative in the data and should reduce the stockpile
                    forestry_surrender_addition = forestry_surrender
                    self.stockpile_balance[year] += forestry_surrender_addition
        
        # Only use stockpile from start year onwards
        if year >= self.stockpile_usage_start_year:
            if net_balance < 0:  # Drawing from stockpile
                shortfall = abs(net_balance)
                
                # First use surplus stockpile
                surplus_used = min(shortfall, self.surplus_balance[year])
                self.surplus_balance[year] -= surplus_used
                # Also reduce the total stockpile balance as surplus is a subset of stockpile
                self.stockpile_balance[year] -= surplus_used
                shortfall -= surplus_used
                
                # If still short, use liquid portion of non-surplus
                if shortfall > 0:
                    non_surplus = max(0, self.stockpile_balance[year] - self.surplus_balance[year])
                    liquid_non_surplus = non_surplus * self.liquidity_factor
                    non_surplus_used = min(shortfall, liquid_non_surplus)
                    
                    # Update stockpile balance
                    self.stockpile_balance[year] -= non_surplus_used
                    
                    # Record borrowing for future payback
                    payback_year = min(max(self.years), year + self.payback_period)
                    if payback_year in self.years:
                        self.borrowed_units.loc[year, payback_year] += non_surplus_used
                
                # Calculate total units used
                units_used_from_stockpile = surplus_used + non_surplus_used
        
        # Handle excess supply (applies to all years)
        if net_balance > 0:  # Adding to stockpile
            # Decide what portion goes to surplus vs non-surplus
            surplus_portion = 0.0  # Or some calculated value based on policy
            non_surplus_portion = net_balance - surplus_portion
            
            # Update both balances
            self.stockpile_balance[year] += net_balance
            self.surplus_balance[year] += surplus_portion
            stockpile_without_forestry[year] += net_balance
        
        # Update results for the given year
        non_surplus = max(0, self.stockpile_balance[year] - self.surplus_balance[year])
        liquid_non_surplus = non_surplus * self.liquidity_factor
        
        self.results.loc[year, 'stockpile_balance'] = self.stockpile_balance[year]
        self.results.loc[year, 'surplus_balance'] = self.surplus_balance[year]
        self.results.loc[year, 'non_surplus_balance'] = non_surplus
        self.results.loc[year, 'liquid_stockpile'] = self.surplus_balance[year] + liquid_non_surplus
        self.results.loc[year, 'other_stockpile'] = non_surplus - liquid_non_surplus
        self.results.loc[year, 'available_units'] = units_used_from_stockpile
        self.results.loc[year, 'surplus_used'] = surplus_used
        self.results.loc[year, 'non_surplus_used'] = non_surplus_used
        self.results.loc[year, 'forestry_held_addition'] = forestry_held_addition
        ...
        # Update subsequent years if they exist
        if year_index < len(self.years) - 1:
            next_year = self.years[year_index + 1]
            self.stockpile_balance[next_year] = self.stockpile_balance[year]
            self.surplus_balance[next_year] = self.surplus_balance[year]
